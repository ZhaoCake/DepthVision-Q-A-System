...
class SessionState:
    ...
    def init_state(self):
        ...
        image2depth = load_tool('ImageToDepth', device='cuda').to_lagent()
        segmentobject = load_tool('SegmentObject', device='cuda').to_lagent()

        action_list = [
            image2depth, segmentobject,
        ]
...
class StreamlitUI:
    ...
    def init_streamlit(self):
        ...
        st.set_page_config(
            layout='wide',
            page_title='深度图像估计',
            page_icon='./resources/asuka_bread.ico')
        st.header(':robot_face: :blue[图像] 深度估计和实例分割 ', divider='rainbow')
    ...
    def setup_sidebar(self):
        ...
        da_prompt = st.sidebar.text_area('数据分析提示词', value="你现在具有从图片分割出某个具体物体以及将图片转换为深度图的能力。当你获取到传入的图像时，你需要使用这两个工具转换图片并且能够使用python代码调用opencv库单独获得分割出的物体的深度图，最后输出这个物体的深度图")
